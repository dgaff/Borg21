...
The image above is the simulator environment. The red and green tracks are two robots or "animats" navigating around obstacles to reach the goal in the top right. The other windows contain information about the classifier system and simulation progress. Not shown are the simulation parameter controls in the Options menu. I haven't tested every aspect of the GUI yet, although I'm not terribly worried about it not working well, since the original simulation code is unchanged and the GUI is just a front-end for that. I just wanted to get the old code running again.

See [CLAUDE.md](CLAUDE.md) for architecture, conventions, and the
reasoning behind the original design. But I want to point out one important note about the code that I'd forgotten. There's a build #define called NETWORK. It means "build the DLCS version" and it's the default. When I started my research, I initially wrote only the LCS implementation, since I needed to see if LCS would even work for a single robot. That was Borg 1.0, and it's backed up separately outside of this repo. That work wasn't sufficient for my masters thesis, since LCS itself wasn't novel, so I wrote version 2, which implemented DLCS. Most of the code was shared, but how simulations were initiated was different. I kept the old LCS code path in, and implemented DLCS with the NETWORK build tag. I never retested the non-NETWORK build path in version 2, because I ended up implementing a way to call the simulator without animats sharing learnings, effectively turning off the "Distributed" part of DLCS and reverting the simulation to one or more robots just running their own LCS. So the old code path wasn't needed anymore. Anyway, Claude found a build error in the non-NETWORK build path, and we chose to fix it. Claude did retest that code path, but I haven't verified it.
